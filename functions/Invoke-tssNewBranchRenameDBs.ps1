  ...
  foreach ($DevDB in $DevDBs) {
    ...
    if ($PSCmdlet.ShouldProcess($DBServer, "Renombrando la base de datos $PRDDB")) {
      try {
        ...
        Invoke-Sqlcmd -ServerInstance $DBServer -Database $PRDDB -Query "EXEC sys.sp_cdc_disable_db"
        Write-Verbose -Message "Matando todas las conexiones y colocando como SINGLE_USER la base de datos $PRDDB"
        Invoke-Sqlcmd -ServerInstance $DBServer -Database "master" -Query "ALTER DATABASE $PRDDB SET SINGLE_USER WITH ROLLBACK IMMEDIATE"
        Write-Verbose -Message "Cambiando de nombre a la base de datos $PRDDB a $PRDOldDB"
        Invoke-Sqlcmd -ServerInstance $DBServer -Database "master" -Query "EXEC sp_renamedb '$PRDDB','$PRDOldDB'"
        Write-Verbose -Message "Regresando la base de datos $PRDOldDB a MULTI_USER"
        Invoke-Sqlcmd -ServerInstance $DBServer -Database "master" -Query "ALTER DATABASE $PRDOldDB SET MULTI_USER"
        Write-Verbose -Message "Colocando la base de datos $PRDOldDB en READ_ONLY"
        Invoke-Sqlcmd -ServerInstance $DBServer -Database "master" -Query "ALTER DATABASE $PRDOldDB SET READ_ONLY"  
      }
      catch {
    ...
    if ($PSCmdlet.ShouldProcess($DBServer, "Renombrando la base de datos $UATDB")) {
      try {
        ...
        Invoke-Sqlcmd -ServerInstance $DBServer -Database $UATDB -Query "EXEC sys.sp_cdc_disable_db"
        Write-Verbose -Message "Matando todas las conexiones y colocando como SINGLE_USER la base de datos $UATDB"
        Invoke-Sqlcmd -ServerInstance $DBServer -Database "master" -Query "ALTER DATABASE $UATDB SET SINGLE_USER WITH ROLLBACK IMMEDIATE"
        Write-Verbose -Message "Cambiando de nombre a la base de datos $UATDB a $PRDDB"
        Invoke-Sqlcmd -ServerInstance $DBServer -Database "master" -Query "EXEC sp_renamedb '$UATDB','$PRDDB'"
        Write-Verbose -Message "Regresando la base de datos $PRDDB a MULTI_USER"
        Invoke-Sqlcmd -ServerInstance $DBServer -Database "master" -Query "ALTER DATABASE $PRDDB SET MULTI_USER"
      }
      catch {
    ...
    if ($PSCmdlet.ShouldProcess($DBServer, "Renombrando la base de datos $UATNewDB")) {
      try {
        ...
        Invoke-Sqlcmd -ServerInstance $DBServer -Database "master" -Query "ALTER DATABASE $UATNewDB SET SINGLE_USER WITH ROLLBACK IMMEDIATE"
        Write-Verbose -Message "Cambiando de nombre a la base de datos $UATNewDB a $UATDB"
        Invoke-Sqlcmd -ServerInstance $DBServer -Database "master" -Query "EXEC sp_renamedb '$UATNewDB','$UATDB'"
        Write-Verbose -Message "Regresando la base de datos $UATDB a MULTI_USER"
        Invoke-Sqlcmd -ServerInstance $DBServer -Database "master" -Query "ALTER DATABASE $UATDB SET MULTI_USER"
      }
      catch {
        Write-Warning -Message "La base de datos $UATDB no pudo ser renombrada."
        Write-Error $_
